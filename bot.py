...
# Предустановленный каталог популярных СМИ для быстрого добавления
PRESET_MEDIA = {
...
# --- Главное Reply-меню внизу экрана ---
...
def get_main_reply_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📢 Канал для постинга"), KeyboardButton(text="👥 Админы")]
        ],
...
async def send_next_post_card(chat_id: int) -> None:
    ...
    card_text = (
        ...
        f"📊 <b>ER / Приоритет:</b> {post['er']:.2f}%\n"
        f"💡 <b>Суть:</b> {html.escape(str(main_idea))}\n"
        f"❓ <b>Инсайт:</b> {html.escape(str(why))}\n"
        f"{link_html}\n"
...
async def cmd_start(message: Message):
    ...
    welcome_text = (
        ...
        "Выберите нужное действие на кнопках ниже или просто <b>перешлите любой текст</b>, чтобы сделать быстрый рерайт!"
    )
    if is_owner:
...
async def menu_parse_modes(message: Message):
    if not await db.is_user_authorized(message.from_user.id):
        return
    await message.answer("🎯 <b>Выберите, откуда собрать свежий контент:</b>", parse_mode=ParseMode.HTML, reply_markup=get_parse_modes_keyboard())
...
async def handle_parse_mode_selected(callback: CallbackQuery):
    ...
    status_msg = await callback.message.answer(f"🔄 <b>Запуск парсинга {mode_names.get(mode)}...</b>", parse_mode=ParseMode.HTML)
    
    await parser.run_parser(mode=mode)
    await status_msg.edit_text("🧠 <b>ИИ-анализ и отбор через Gemini...</b>", parse_mode=ParseMode.HTML)
...
async def menu_media(message: Message):
    if not await db.is_user_authorized(message.from_user.id):
        return
    await message.answer("📰 <b>Управление СМИ и новостными сайтами</b>\n\nВы можете добавить популярные СМИ в 1 клик из каталога или подключить любую свою RSS-ссылку командой:\n<code>/add_site Название https://site.ru/rss</code>", parse_mode=ParseMode.HTML, reply_markup=get_media_menu_keyboard())

@router.callback_query(F.data == "media_catalog")
async def handle_media_catalog(callback: CallbackQuery):
    await callback.message.edit_text("⚡️ <b>Каталог популярных СМИ (нажмите, чтобы добавить):</b>", parse_mode=ParseMode.HTML, reply_markup=get_media_catalog_keyboard())
...
async def handle_media_my_list(callback: CallbackQuery):
    ...
    text = "📋 <b>Ваши подключенные СМИ и сайты:</b>\n\n"
    for idx, s in enumerate(sources, start=1):
        text += f"{idx}. <b>{html.escape(s['name'])}</b>\n🔗 <code>{html.escape(s['url'])}</code>\n\n"
    text += "<i>Чтобы удалить сайт, отправьте:</i>\n<code>/del_site URL_сайта</code>"
...
async def handle_media_back(callback: CallbackQuery):
    await callback.message.edit_text("📰 <b>Управление СМИ и новостными сайтами:</b>", parse_mode=ParseMode.HTML, reply_markup=get_media_menu_keyboard())
...
async def cmd_add_site(message: Message, command: CommandObject):
    ...
    if not command.args or len(command.args.split()) < 2:
        await message.answer("⚠️ Формат: <code>/add_site Название https://site.ru/rss</code>\nПример: <code>/add_site Хабр https://habr.com/ru/rss/all/all/</code>", parse_mode=ParseMode.HTML)
        return
    ...
    if success:
        await message.answer(f"✅ Сайт <b>{html.escape(name)}</b> добавлен в мониторинг!", parse_mode=ParseMode.HTML)
    else:
        await message.answer(f"ℹ️ Этот RSS-поток уже есть в списке.", parse_mode=ParseMode.HTML)
...
async def cmd_del_site(message: Message, command: CommandObject):
    if not await db.is_user_authorized(message.from_user.id):
        return
    if not command.args:
        await message.answer("⚠️ Укажите URL: <code>/del_site https://site.ru/rss</code>", parse_mode=ParseMode.HTML)
        return

    url = command.args.strip()
    success = await db.delete_media_source(url)
    if success:
        await message.answer(f"🗑 Источник удален!", parse_mode=ParseMode.HTML)
    else:
        await message.answer("❌ Источник с таким URL не найден.", parse_mode=ParseMode.HTML)
...
async def menu_tg_channels(message: Message):
    ...
    if not channels:
        await message.answer("📭 Список каналов пуст.\nДобавьте канал: <code>/add_channel username</code>", parse_mode=ParseMode.HTML)
        return
    ...
    text += "\n➕ <i>Добавить:</i> <code>/add_channel username</code>\n🗑 <i>Удалить:</i> <code>/del_channel username</code>"
...
async def menu_set_channel(message: Message, command: CommandObject = None):
    ...
    await message.answer(
        f"📢 <b>Текущий канал для публикаций:</b> <code>{curr}</code>\n\n"
        f"Чтобы изменить, отправьте команду:\n<code>/set_channel @my_channel_username</code>\n\n"
        f"<i>(Убедитесь, что бот добавлен администратором в этот канал)</i>",
        parse_mode=ParseMode.HTML
    )
...
async def menu_admins(message: Message):
    ...
    text += "\n➕ <i>Выдать доступ:</i> <code>/add_admin ID</code>\n🗑 <i>Забрать доступ:</i> <code>/del_admin ID</code>"
...
@router.message(F.text & ~F.text.startswith("/") & ~F.text.in_({"🚀 Запустить парсинг", "📱 ТГ-каналы", "📰 СМИ и Сайты", "📢 Канал для постинга", "👥 Админы", "🔙 Главное меню"}))
...
async def handle_custom_rewrite(callback: CallbackQuery):
    ...
    style_names = {"expert": "💼 Экспертный", "tldr": "⚡️ TL;DR", "provocative": "🔥 Дерзкий", "guide": "🧵 Гайд"}
...
# --- Обработка очереди спарсенных постов ---
...
async def handle_rewrite_style(callback: CallbackQuery):
    ...
    await callback.answer(f"⏳ Генерирую: {style_names.get(style, '')}...")
...
async def auto_parser_loop():
    while True:
        ...
        try:
            new_parsed = await parser.run_parser(mode="all")
            if new_parsed > 0:
                await analyzer.process_new_posts()
                ...
                if chosen and config.ADMIN_ID:
                    await bot.send_message(
                        config.ADMIN_ID,
                        f"🔔 <b>Автопарсинг:</b> Найдено <b>{len(chosen)}</b> новых материалов (ТГ + СМИ)!\nНажмите кнопку «🚀 Запустить парсинг» для просмотра.",
                        parse_mode=ParseMode.HTML
                    )
        except Exception as e:
